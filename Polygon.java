...
public class Polygon {
    ...
    private Polygon(List<Line> sides, BoundingBox boundingBox, List<Point> _vertexes) {
        _sides = sides;
        _boundingBox = boundingBox;
        vertexes= _vertexes;

        xRange=_boundingBox.xMax;
        yRange = _boundingBox.yMax;
    }
    ...
    /**
     *  Function to find centertroid/ center point of polygon
     *
     * @author Casey Charlesworth (caseyleigh54@yahoo.com)</br>
     * @return Point, center point of polygon
     */
    public Point centerPoint() {
        ...
        // gets X values and converts to list, of type Object
        xPoints = vertexes.stream().map(Point::getX).collect(Collectors.toList());
        // gets Y values and converts to list, of type Object
        yPoints = vertexes.stream().map(Point::getY).collect(Collectors.toList());

        // calculates centroid
        for(int i = 0;i < xPoints.size() - 1;i++) {

            x = (double)xPoints.get(i);
            xNext = (double)xPoints.get(i+1);
            y = (double)yPoints.get(i);
            yNext = (double)yPoints.get(i+1);

            A = A + (x * yNext) - (xNext * y);

            xCenter = xCenter + (x + xNext) * ((x * yNext) - (xNext * y));
            yCenter= yCenter + (y + yNext) * ((x * yNext) - (xNext * y));
        ...
        Point centerPt = new Point(xCenter, yCenter);
        return centerPt;

    }

    /**
     *  Function to return X points in integer format for display
     *
     * @author Casey Charlesworth (caseyleigh54@yahoo.com)</br>
     * @return int[] X points of polygon
     */
    public int[] getXPoints() {

        List xPoints = vertexes.stream().map(Point::getX).collect(Collectors.toList());
        Object[] Arr = xPoints.toArray();
        int[] y= new int[xPoints.size()];
        Double temp;
    ...
    /**
     *  Function to return Y points in integer format for display
     *
     * @author Casey Charlesworth (caseyleigh54@yahoo.com)</br>
     * @return int[] Y points of polygon
     */
    public int[] getYPoints() {

        List yPoints = vertexes.stream().map(Point::getY).collect(Collectors.toList());
        Object[] Arr = yPoints.toArray();
        Double temp;
